...
`;
  ...
  function compileShader(type, source) {
    ...
    if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
      console.warn('DarkVeil shader compile:', gl.getShaderInfoLog(shader));
  ...
  if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
    console.warn('DarkVeil program link:', gl.getProgramInfoLog(program));
    return;
  }
  ...
  function loop() {
    resize();
    const time = (performance.now() - startTime) / 1000 * config.speed;
